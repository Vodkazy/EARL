...
try:
    ...
    model = gensim.models.KeyedVectors.load_word2vec_format('../data/lexvec.commoncrawl.300d.W.pos.neg3.vectors')
except Exception,e:
    print e
    sys.exit(1)            
...
def textMatch():
    ...
    for chunk in chunks:
         if chunk['class'] == 'entity':
             res = es.search(index="dbentityindex11", doc_type="records", body={"query":{"multi_match":{"query":chunk['chunk'],"fields":["wikidataLabel", "dbpediaLabel^1.5"]}},"size":200},request_timeout=60)
             _topkents = []
             topkents = []
